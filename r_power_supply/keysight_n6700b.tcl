...
itcl::class keysight_n6700b {
  ...
  variable chan; # channel number, without suffixes, used in communication with device
  constructor {args} {
    chain {*}$args

    # parse channel name, range (H/L) and polarity pins (:P45):
    if {![regexp {([0-4])([HL]?)(:P([2-7])([2-7]))?} $dev_chan x chan range p0 sw_pos sw_neg]} {
    ...
    # detect module type:
    set mod [Device2::ask $dev_name "syst:chan:mod? (@$chan)"]
    switch -glob -- $mod {
      N6731B {
        set min_i 0.06
        set min_i_step 1e-2
      ...
      N676[12]A {
        ...
        switch -- $range {
          H {
            Device2::ask $dev_name "curr:rang 1,(@$chan)"
            set min_i 1e-3;
            set min_i_step 1e-4
            set i_prec 1.2e-3; # we can set 0 and be at 0.001
          }
          L {
            Device2::ask $dev_name "curr:rang 0.09,(@$chan)"
            set min_i 1e-4;
            set min_i_step 2e-6
            set i_prec 1.2e-4
          }
    ...
    set max_i [Device2::ask $dev_name "curr:rang? (@$chan)"]
    set max_v [Device2::ask $dev_name "volt:rang? (@$chan)"]
    set min_v 0
  ...
  method check_power {} {
    ...
    set data [Device2::ask $dev_name "dig:inp:data?"]
    if { [get_pin $data 1] } { return }
    # revert polarity and try again:
    set pol [string equal [Device2::ask $dev_name "dig:pin1:pol?"] "NEG"]
    Device2::ask $dev_name "dig:pin1:pol [expr $pol?{POS}:{NEG}]"
    set data [Device2::ask $dev_name "dig:inp:data?"]
    if { [get_pin $data 1] } { return }
    # fail:
  ...
  method get_pin {data n} {
    set pol [string equal [Device2::ask $dev_name "dig:pin$n:pol?"] "NEG"]
  ...
  method get_pol {chan sw_pos sw_neg} {
    ...
    set data [expr int([Device2::ask $dev_name "dig:inp:data?"])]
  ...
  method set_pol {pol chan sw_pos sw_neg} {
    ...
    if {$data1 != $data } {Device2::ask $dev_name "DIG:OUTP:DATA $data1"}
    # Check new state:
    if { $data1 != [Device2::ask $dev_name "DIG:INP:DATA?"] } {
  ...
  method set_volt {val} {
    ...
    if {($sw_pos == {} || $sw_neg == {}) || $val == 0} {
      Device2::ask $dev_name "volt $val,(@$chan)"
      return
    }
    # set channel polarity, set current
    set_pol $val $chan $sw_pos $sw_neg
    Device2::ask $dev_name "volt [expr abs($val)],(@$chan)"
  }

  method set_curr {val} {
    # No polarity switch or zero current:
    if {($sw_pos=={} || $sw_neg=={}) || $val == 0} {
      Device2::ask $dev_name "curr $val,(@$chan)"
      return
    }
    # set channel polarity, set current
    set_pol $val $chan $sw_pos $sw_neg
    Device2::ask $dev_name "curr [expr abs($val)],(@$chan)"
  }

  method get_volt {} {
    set val [Device2::ask $dev_name "meas:volt? (@$chan)"]
    if {$sw_pos!={} && $sw_neg!={}} {
      set val [expr {$val*[get_pol $chan $sw_pos $sw_neg]}] }
    return $val
  }
  method get_curr_abs {} {
    return [Device2::ask $dev_name "meas:curr? (@$chan)"]
  }
  method get_curr {} {
  ...
  method set_ovp  {val} {
    Device2::ask $dev_name "volt $val,(@$chan)"
    Device2::ask $dev_name "volt:prot $val,(@$chan)"
  }
  method set_ocp  {val} {
    Device2::ask $dev_name "curr $val,(@$chan)"
    Device2::ask $dev_name "curr:prot $val,(@$chan)"
  }

  method cc_reset {} {
    set oc [Device2::ask $dev_name "stat:oper:cond? (@$chan)"]
    set qc [Device2::ask $dev_name "stat:ques:cond? (@$chan)"]
    ...
    if {$oc&4 && $qc&1} {
      Device2::ask $dev_name "curr 0,(@$chan)"
      after 100
      Device2::ask $dev_name "outp:prot:cle (@$chan)"
      after 100
      return
    }
    ...
    if {$oc&4} {
      Device2::ask $dev_name "curr 0,(@$chan)"
      after 100
      Device2::ask $dev_name "outp on,(@$chan)"
      after 100
      return
    }
  ...
  method get_stat {} {
    # error states
    set n [Device2::ask $dev_name "stat:ques:cond? (@$chan)"]
    ...
    set n [Device2::ask $dev_name "stat:oper:cond? (@$chan)"]
